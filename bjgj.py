...
class BeijingBusApi(object):
    ...
    def check_update(self):
        return etree_xpath_children_to_dict_list(ET.fromstring(self.api_open(u'/aiguang/bjgj.c?m=checkUpdate&version=2')), u'//line')
    ...
    def get_busline_realtime_info(self, id, no):
        ...
        def _hy_anon_fn_21():
            f_1240 = (lambda it: decrypt_bus_realtime_info(it))
            for v_1239 in etree_xpath_children_to_dict_list(ET.fromstring(self.api_open(u'/bus.php?city=%E5%8C%97%E4%BA%AC&id={0}&no={1}&type={2}&encrypt={3}&versionid=2'.format(id, no, 1L, 1L), u'http://bjgj.aibang.com:8899')), u'//data/bus'):
                yield f_1240(v_1239)
        return list(_hy_anon_fn_21())
...
def main(*args):
    ...
    def _hy_anon_fn_24():
        f_1243 = (lambda it: it[u'id'])
        for v_1242 in inspect(b.check_update()):
            yield f_1243(v_1242)
    print len(list(_hy_anon_fn_24()))
